...
<div class="relative p-4 w-full max-w-md max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="modal_main">
                <form class="space-y-4" action="{{ route('populars.update', ['popular' => $populars->id])}}" method='post'>
                @csrf
                    <div>
                ...
                <option value="{{ $musica->id }}" 
                        @if($musica->id == $populars->id_musica) selected @endif>
                    {{ $musica->nome }}
                </option>
            @endforeach
                    </select>
            ...
            @foreach($artistas as $artista)
                <option value="{{ $artista->id }}" 
                        @if($artista->id == $populars->id_artista) selected @endif>
                    {{ $artista->nome }}
                </option>
            @endforeach
                        </select>
                ...
                <option value="{{ $genero->id }}" 
                        @if($genero->id == $populars->id_genero) selected @endif>
                    {{ $genero->nome }}
                </option>
            @endforeach
</select>
...
@foreach($albuns as $album)
                <option value="{{ $album->id }}" 
                        @if($album->id == $populars->id_album) selected @endif>
                    {{ $album->nome }}
                </option>
            @endforeach
</select>
